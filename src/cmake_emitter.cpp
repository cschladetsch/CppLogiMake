...
// the workspace (a system library, or a package located via a
// project-level package/2 fact — see prolog/targets.pl and
// emitCMakeLists's find_package preamble below). Emitting it as
// INTERFACE is the only shape that's both valid CMake
...
std::string emitCMakeLists(const std::vector<TargetInfo>& targets,
    ...
    // package(Name, Mode) facts must be located before any target that
    // links against the imported targets they provide (e.g.
    // GTest::gtest_main) — see prolog/targets.pl. Emitted right after the
    // project()/standard preamble and before the first add_executable/
    // add_library, one find_package call per distinct package.
    if (!packages.empty()) {
